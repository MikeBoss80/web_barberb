...
def modules_sidebar(context):
    ...
    if current_role == 'Administrador':
        return [
            ('admin_module:main', 'grid-fill' , 'Dashboard'),
            ...
            ('admin_module:admin_solicitudes_list','clipboard-check','Solicitudes'),
    ...
    if current_role == 'Barbero':
        return [
            ('admin_module:main', 'grid-fill' , 'Dashboard'),
            ('admin_module:citas','calendar2-date-fill','Citas'),
            ('admin_module:barber_solicitudes_list','clipboard-check','Solicitudes'),
            ('admin_module:soporte','info-circle','Soporte'),
        ]
    return [
